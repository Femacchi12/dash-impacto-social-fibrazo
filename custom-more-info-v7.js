...
renderCities=function(){
 ...
 const baseLabel=view==='people'?'Base: '+fmt(total)+' alumnos conectados':view==='foundations'?'Base: '+fmt(total)+' fundaciones registradas':'Base: '+fmt(total)+' escuelas analizadas';
 $('#cityChart').innerHTML=arr.length?'<p class="scope-note">'+baseLabel+'</p>'+arr.map(x=>'<div class="bar-row"><span>'+esc(x[0])+'</span><div class="track"><i style="width:'+x[1]/max*100+'%"></i></div><b><span>'+fmt(x[1])+'</span><em>'+percentage(x[1])+'</em></b></div>').join(''):'<p class="empty">Sin resultados.</p>';
};
