...
renderCities=function(){
 ...
 $('#cityChart').innerHTML=arr.length?arr.map(x=>'<div class="bar-row"><span>'+esc(x[0])+'</span><div class="track"><i style="width:'+x[1]/max*100+'%"></i></div><b><span>'+fmt(x[1])+'</span><em>'+percentage(x[1])+'</em></b></div>').join(''):'<p class="empty">Sin resultados.</p>';
};
...
renderSizes=function(){
 ...
 const percentage=value=>((value/total)*100).toFixed(1).replace('.',',')+'%';
 $('#sizeChart').innerHTML=order.map(k=>'<div class="size-row"><span>'+k+'</span><div class="track"><i style="width:'+counts[k]/max*100+'%"></i></div><b><span>'+counts[k]+'</span><em>'+percentage(counts[k])+'</em></b></div>').join('');
