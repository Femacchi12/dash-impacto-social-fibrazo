...
renderConnectivity=function(){
 ...
 $('#qualityChart').innerHTML=group(rs,'quality').map(x=>'<div class="quality"><b>'+x[1]+'</b><small>'+esc(x[0])+'</small><em>'+percentage(x[1])+'</em></div>').join('')||'<p class="empty">Sin datos.</p>';
};
...
renderSizes=function(){
 ...
 $('#sizeSummary').innerHTML='<article><span>Promedio por escuela</span><b>'+fmt(avg)+'</b></article><article><span>Escuelas analizadas</span><b>'+fmt(rs.length)+'</b></article><article><span>Institución con más alumnos</span><b>'+(largest?fmt(largest.people)+' alumnos':'0 alumnos')+'</b><small>'+(largest?esc(largest.institution):'Sin resultados')+'</small></article>';
};
